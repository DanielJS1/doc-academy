-- Migração para suporte a respostas parcialmente corretas (+4 XP) e comentários
begin;

alter table public.academy_attempts add column if not exists partial_text_ids jsonb not null default '[]';

-- Atualiza academy_mutate para armazenar partial_text_ids e conceder +4 XP para acertos parciais
...
begin
 ...
 if op='save-resource' then
  ...
  update public.academy_attempts set correct_text_ids=coalesce(command->'correctTextIds','[]'), partial_text_ids=coalesce(command->'partialTextIds','[]') where id=attempt.id;
  for question in select value from jsonb_array_elements(attempt.snapshot->'questions') loop
   ...
   elsif (question->>'type'='text' and coalesce(command->'partialTextIds','[]') ? (question->>'id')) then
    perform academy_award_xp(attempt.user_id,attempt.course_id,'question:'||(question->>'id'),4,'Acerto parcial · '||(attempt.snapshot->>'title'));
   end if;
  end loop;
  if approved then
  ...
  if command->>'kind'='departments' then
   update public.academy_settings set departments=case when old_name is null then departments || jsonb_build_array(new_name) else (select jsonb_agg(case when value=old_name then new_name else value end) from jsonb_array_elements_text(departments)) end where id=true;
   if old_name is not null then update public.academy_profiles set department=new_name where department=old_name; end if;
  else
   update public.academy_settings set products=case when old_name is null then products || jsonb_build_array(new_name) else (select jsonb_agg(case when value=old_name then new_name else value end) from jsonb_array_elements_text(products)) end where id=true;
