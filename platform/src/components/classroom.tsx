...
export function Classroom({ id, initialLesson, initialPosition = 0, preview = false }: { id: string; initialLesson?: string; initialPosition?: number; preview?: boolean }) {
  ...
  return <div className="page-enter classroom-page"><div className="classroom-top-actions">{preview && <Link className="back-link" href={`/admin/cursos/${id}`}><ArrowLeft size={15}/>Voltar ao editor</Link>}<Button variant="ghost" size="sm" className="focus-toggle-btn" onClick={() => setFocusMode(v => !v)} title={focusMode ? "Exibir a lateral completa" : "Modo foco (encolher lateral)"}>{focusMode ? <PanelRightOpen size={16}/> : <PanelRightClose size={16}/>}<span>{focusMode ? "Exibir a lateral" : "Modo foco (encolher lateral)"}</span></Button>{lesson.type === "video" && <Button variant="secondary" size="sm" className="notepad-toggle" aria-expanded={notesOpen} onClick={() => setNotesOpen(value => !value)}><NotebookPen size={16}/>{notesOpen ? "Recolher caderno" : "Abrir caderno de anotações"}</Button>}</div>{preview && <div className="notice-bar"><Eye size={17}/><span>Pré-visualização. Sua participação, XP e avaliações não serão alterados.</span></div>}<div className={`player-grid ${focusMode ? "focus-mode" : ""} ${lesson.type === "video" ? "video-theater" : ""}`}><div className={`classroom-stage ${lesson.type === "video" && notesOpen ? "notes-open" : ""}`}><div className="classroom-stage-primary">
    ...
    {focusMode ? (
      ...
      <aside className="curriculum panel">
        ...
        <div className="curriculum-list" role="region" aria-label="Lista de aulas" tabIndex={0}>{course.lessons.map((item, position) => (
          <div key={item.id}>
