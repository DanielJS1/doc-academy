...
export function Classroom({ id, initialLesson, initialPosition = 0, preview = false }: { id: string; initialLesson?: string; initialPosition?: number; preview?: boolean }) {
  ...
  const [selected, setSelected] = useState(initialLesson || ""); const [answers, setAnswers] = useState<Record<string, string>>({}); const [retrying, setRetrying] = useState(false); const [focusMode, setFocusMode] = useState(true); const [notesOpen, setNotesOpen] = useState(false); const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
