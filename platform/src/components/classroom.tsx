  ...
  return <div ref={pageRef} className="page-enter classroom-page">
    <div className="classroom-top-actions">
      ...
      <div className="classroom-header-controls">
        ...
        <Button variant="ghost" size="sm" className="focus-toggle-btn" aria-pressed={focusMode} onClick={() => { if (focusMode) setNotesOpen(false); setFocusMode(value => !value); }} title={focusMode ? "Exibir a lateral completa" : "Modo foco (encolher lateral)"}>
          {focusMode ? <PanelRightOpen size={16}/> : <PanelRightClose size={16}/>}
          <span>{focusMode ? "Exibir a lateral" : "Modo foco (encolher lateral)"}</span>
        </Button>
        {lesson.type === "video" && <Button variant="secondary" size="sm" className="notepad-toggle" aria-expanded={notesOpen} onClick={() => { if (!notesOpen) setFocusMode(true); setNotesOpen(value => !value); }}><NotebookPen size={16}/>{notesOpen ? "Recolher caderno" : "Abrir caderno de anotações"}</Button>}
      </div>
    </div>
      ...
      >
        <div className="dock-header">
          <button
            ...
            onClick={() => { setNotesOpen(false); setFocusMode(false); }}
