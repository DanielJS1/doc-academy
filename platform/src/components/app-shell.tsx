...
import { ArrowUpRight, Bell, BookOpen, ChevronRight, CircleHelp, Command, GraduationCap, Home, Menu, Moon, Search, Settings2, Sparkles, Sun, Trophy, Users, X } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { state, update, theme, toggleTheme, storageError } = useAcademy();
  ...
  return <div className="app">
    ...
    <Link href="/" className="brand" aria-label="DOC-Academy — início"><span><strong>DOC-Academy</strong><small>CONHECIMENTO QUE TRANSFORMA</small></span></Link>
      ...
      <span className="nav-label manage-label">GESTÃO</span>
      <nav><Link href="/equipe" className={`nav-item ${active("/equipe") ? "active" : ""}`} aria-current={active("/equipe") ? "page" : undefined}><Users size={19}/> Minha equipe</Link><Link href="/admin" className={`nav-item ${active("/admin") ? "active" : ""}`} aria-current={active("/admin") ? "page" : undefined}><Settings2 size={19}/> Administração</Link></nav>
      <div className="sidebar-bottom"><div className="sidebar-note"><span className="little-star"><Sparkles size={17}/></span><strong>Conhecimento abre caminhos.</strong><p>Um novo aprendizado.<br/>Uma nova possibilidade.</p><Link href="/aprender">Explore os cursos <ArrowUpRight size={16}/></Link></div><Link className="help-link" href="/sobre"><CircleHelp size={17}/> Sobre esta demonstração</Link><div className="sidebar-footer"><div className="brand-block"><span className="demaria">de<span>maria</span><i/></span><img className="brand-official" src="/doc-academy-logo-oficial.png" alt="DOC-Academy logo"/></div><span>Aprender. Evoluir.</span></div></div>
    </aside>
    <div className="app-content"><header className="topbar"><div className="breadcrumb"><Button variant="ghost" size="icon" className="mobile-menu" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Abrir navegação" aria-expanded={mobileOpen}><Menu size={22}/></Button><span>Meu espaço</span><ChevronRight size={14}/><strong>{title}</strong></div>
      <form className="global-search" role="search" onSubmit={event => { event.preventDefault(); router.push(`/aprender?busca=${encodeURIComponent(search)}`); }}><Search size={17}/><input ref={searchRef} aria-label="Buscar cursos" placeholder="O que você quer aprender?" value={search} onChange={event => setSearch(event.target.value)}/><kbd><Command size={10}/> K</kbd></form>
      <div className="topbar-actions"><Link href="/sobre" className="demo-tag"><span/> Demonstração</Link><Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={theme === "light" ? "Ativar tema escuro" : "Ativar tema claro"}>{theme === "light" ? <Moon size={19}/> : <Sun size={19}/>}</Button><div className="notifications"><Button variant="ghost" size="icon" aria-label="Notificações" aria-expanded={noticesOpen} onClick={() => setNoticesOpen(!noticesOpen)}><Bell size={19}/>{unread > 0 && <span className="notification-dot"/>}</Button>{noticesOpen && <section className="notification-panel" aria-label="Novidades"><div className="section-title"><h3>Novidades</h3><Button size="icon" variant="ghost" aria-label="Fechar notificações" onClick={() => setNoticesOpen(false)}><X size={16}/></Button></div>{notices.map(item => <Link href={item.href} key={item.id} onClick={() => { update(current => ({ ...current, readNotices: Array.from(new Set([...current.readNotices, item.id])) })); setNoticesOpen(false); }}><span className="notification-symbol"><Sparkles size={16}/></span><span><strong>{item.title}</strong><small>{item.text}</small></span>{!state.readNotices.includes(item.id) && <i/>}</Link>)}</section>}</div><span className="topbar-divider"/><Link href="/conquistas" className="profile" aria-label="Ver minha evolução"><span className="avatar avatar-daniel">D</span><span><strong>Daniel</strong><small>Meu aprendizado</small></span></Link></div></header>
      {storageError && <div className="storage-warning" role="alert">Não foi possível salvar no navegador. Alterações desta sessão podem ser perdidas ao fechar a página.</div>}
      <main id="conteudo" className="main-content">{children}</main><footer className="main-footer"><span>DOC-Academy <span>·</span> Feito para evoluir com você.</span><span>Prévia local · conteúdos e estatísticas de exemplo</span></footer>
    </div>
  </div>;
}
