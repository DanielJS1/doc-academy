...
export function VimeoLesson({course,lesson,preview,nextTitle,onNext}:{course:Course;lesson:Lesson;preview:boolean;nextTitle?:string;onNext?:()=>void}){
 ...
 useEffect(()=>{
  ...
  const tick=(event:{seconds:number;duration:number})=>{const near=isVideoNearEnd(event.seconds,event.duration);setNearEnd(near);const entered=near&&!wasNearEnd;wasNearEnd=near;void record(entered);};const flush=()=>void record(true);const onError=()=>setError("O Vimeo não conseguiu abrir esta aula. Avise o administrador.");
  player.on("timeupdate",tick);player.on("seeked",flush);player.on("pause",flush);player.on("ended",flush);player.on("error",onError);
  return()=>{active=false;player.off("timeupdate",tick);player.off("seeked",flush);player.off("pause",flush);player.off("ended",flush);player.off("error",onError);};
